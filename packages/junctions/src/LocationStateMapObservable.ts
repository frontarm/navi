...
import { JunctionRoute, Route, RouteType, RouteStatus } from './Route'
import { RouterMapOptions } from './Router'
import { RouterLocationStateMap } from './Maps'
...
export class LocationStateMapObservable implements Observable<RouterLocationStateMap> {
    private cachedSiteMap?: RouterLocationStateMap
    private rootJunction: Junction
    private rootMapping: AbsoluteMapping
    private observers: Observer<RouterLocationStateMap>[]
    ...
    subscribe(
        onNextOrObserver: Observer<RouterLocationStateMap> | ((value: RouterLocationStateMap) => void),
        ...
        let observer: Observer<RouterLocationStateMap> = 
    ...
    getValue(): RouterLocationStateMap {
    ...
    private handleUnsubscribe = (observer?: Observer<RouterLocationStateMap>) => {
        if (observer) {
    ...
    private refresh = () => {
        ...
        while (i < this.queueURLs.length) {
            ...
            if (url.substr(-1) !== '/' && lastRoute && lastRoute.type !== RouteType.Junction) {
                let secondLastRoute = (route!.remainingRoutes[route!.remainingRoutes.length - 2] as JunctionRoute) || route!
            ...
            if (
                ...
                // linking to them.
                (item.fromURL && !doesLinkTo(this.queueItems[this.queueURLs.indexOf(item.fromURL)], url)) ||
            ...
            if (
                ...
                for (let j = 0; j < mappings.length; j++) {
                    this.addToQueue(joinPaths(url, mappings[j].pattern), item.depth + 1, url)
                }
            }
        ...
        for (let i = 0; i < this.queueURLs.length; i++) {
            let url = this.queueURLs[i]
            let item = this.queueItems[i]
            this.cachedSiteMap[joinPaths(url, '/')] = item.routeCache!
        }
    ...
    private addToQueue(url, depth, fromURL?) {
        if (this.queueURLs.indexOf(url) === -1 && (!this.options.maxDepth || depth <= this.options.maxDepth)) {
...
function doesLinkTo(queueItem: QueueItem | undefined, url: string) {
    let lastRoute =
        queueItem &&
        queueItem.lastRouteCache

    return (
        lastRoute &&
        lastRoute.status === RouteStatus.Ready && (
            (lastRoute.type == RouteType.Redirect && lastRoute.to && createURL(lastRoute.to) === url) ||
            (
                lastRoute.type === RouteType.Junction &&
                lastRoute.junction.patterns.indexOf(url.substr(lastRoute.url.length)) !== -1
            )
        )
    )
}